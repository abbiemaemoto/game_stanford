...
export default function WordleGame() {
  ...
  const getProdigyPoints = () => {
    if (gameStatus === "won") {
      return 7 - attempts.length // +6 for first attempt, +5 for second, etc., +1 for sixth
    }
  ...
  const getPointsMessage = () => {
    ...
    if (gameStatus === "won") {
      return `You solved it in ${attempts.length} attempt${attempts.length !== 1 ? 's' : ''}! You earned +${points} Prodigy Points!`
    } else {
      return `You earned +${points} Prodigy Point for trying!`
    }
  }

  const handleGameComplete = () => {
    // Navigate to fall shareout
    router.push("/game/activities/8-fall-finals/fall-shareout1")
  }
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-400 rounded-2xl p-6 shadow-lg text-center">
              ...
              {gameStatus === "won" && (
                <p className="text-lg text-blue-600">
                  Point Scale: +6 for 1st attempt → +1 for 6th attempt
                </p>
              )}
            </div>
